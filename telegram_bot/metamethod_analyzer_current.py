...
3. СЛОИ - ОТКУДА ИДЁТ ПРОГРАММА (структурированный анализ по трём слоям):

   ВАЖНАЯ ТЕОРИЯ О СЛОЯХ:
   Слой — это то, где сформировалась ПЕРВОПРИЧИНА. Не то, с чем человек сталкивается сейчас, а то, куда КОРНЯМИ уходит событие.

   **Первый слой: Текущая жизнь** (от внутриутробного периода до сегодня)
   - Всё, что происходило с мамой во время беременности, отражается на человеке
   - Детство, подростковый возраст, взрослая жизнь
   - Ссоры родителей, нападения, увиденное и услышанное
   - ВАЖНО: Первый слой КРАЙНЕ РЕДКО является причиной! Как правило, это следствие.
   - В этом слое заряд формируется как ЗАКАЗАННЫЙ ОПЫТ души

   **Второй слой: Родовой** (работаем с 3-5 поколениями, максимум 7)
   Три варианта формирования заряда:

   а) Я проживаю программу кого-то из предков (носитель программы)
      - Женщины из поколения в поколение теряли мужей
      - Женщины всё тащили на себе
      - Программа кого-то из бабушек/дедушек повторяется во мне

   б) Я заимствовал/заимствовала программу, потому что есть энергетический долг перед предками
      - Я их не спасла, предала, осудила в прошлом
      - Непрощенка к ним сформировала заряд
      - Прихожу в род, чтобы проживать с ними то, с чем не согласилась когда-то

   в) У моего рода есть долги перед другими людьми
      - Род служил инквизицией, лишал жизни людей со способностями
      - Род прощал себя: "я могу спасать, а могу и не делать"
      - Я прихожу отрабатывать эти родовые долги

   **Третий слой: Инкарнационный/Прошлые жизни** (опыты других воплощений)
   - Заряд формируется в прошлых жизнях и кочует из воплощения в воплощение
   - Опыты рабства, заключения, феодализма, королевских семей
   - Инкарнации могут быть: в людей, животных, птиц, растения, камни, кристаллы, непроявленных существ
   - Человек с инкарнацией в растения/птицы: тонкая душевная организация, творческий, не от мира сего
   - Человек с инкарнацией в камень: медленный, медлительный, не спешит, планомерный
   - Человек с инкарнацией в непроявленных существ (гномы): страшно проявляться, не чувствует тела, в фантазиях

   ФОРМАТ АНАЛИЗА ДЛЯ ЗАПРОСА (4 ПОДПУНКТА):

   • Текущая жизнь: [Основной источник — родовой/инкарнационный уровень]

   • Родовой слой: [КОНКРЕТНОЕ поколение и линия! Например: "Эта программа идёт от бабушки по маминой линии. Бабушка всю жизнь старалась быть полезной семье, забывая о собственных нуждах. Мама переняла этот паттерн, и теперь он проявляется у [имя]."]

   • Конкретные родовые сценарии: [С ЦИТАТОЙ! Например: "По маминой линии есть сценарий: «я должна жертвовать собой ради семьи». Женщины в роду считали, что забота о себе — это эгоизм, что усиливает выгорание."]

   • Инкарнационный слой: [Конкретный опыт из прошлых жизней или "Опыт не прослеживается напрямую"]

   Примеры ХОРОШИХ формулировок:
   - "Программа от бабушки по маминой линии — там опыт подавления чувств"
   - "Идёт от прабабушки по папиной линии, времена феодализма"
   - "Род служил при дворе, опыт королевских семей по папиной линии"
...
• Родовой слой: [КОНКРЕТНОЕ поколение и линия с описанием опыта]

• Конкретные родовые сценарии: [С ЦИТАТОЙ сценария в роду]
...
"""
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[{"role": "user", "content": prompt}],
            temperature=0.7,
            max_tokens=1200  # Увеличили для детального описания слоёв программ
        )
...
ОБЯЗАТЕЛЬНО ИСПОЛЬЗУЙ ЦВЕТНЫЕ ЭМОДЗИ ПЕРЕД КАЖДОЙ ЧАКРОЙ:
🔴 - для Муладхары (красная, 1-я чакра)
🟠 - для Свадхистханы (оранжевая, 2-я чакра)
🟡 - для Манипуры (желтая, 3-я чакра)
💚 - для Анахаты (зеленая, 4-я чакра)
💙 - для Вишудхи (голубая, 5-я чакра)
💜 - для Аджны (фиолетовая, 6-я чакра)
🤍 - для Сахасрары (белая, 7-я чакра)
...
• Инкарнационный слой
[возьми ИНКАРНАЦИОННЫЙ СЛОЙ из части 2, или напиши "Опыт не прослеживается напрямую", 1-2 предложения]
