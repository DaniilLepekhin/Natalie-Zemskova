...
class MetaMethodAnalyzer:
    ...
    def analyze(self, request_text: str, username: str) -> tuple:
        ...
        step2_result, usage2 = self._step2_ancestral_patterns(request_text, step1_result, username)
...
2. ГЛАВНЫЕ ПРОГРАММЫ (МИНИМУМ 3, а лучше 4-5 штук в кавычках от 3-го лица с использованием имени {username})
3. УРОКИ ДУШИ (МИНИМУМ 3 урока, каждый — конкретный и практический, НЕ абстрактный)
4. ЧТО МЕНЯТЬ (минимум 3 пункта с конкретикой)

ВАЖНО ПРО ПЕРСОНАЛИЗАЦИЮ:
Все программы должны быть от 3-го лица с именем {username}, с феминизацией где нужно.
НЕ используй "я" - используй имя {username}!

СТИЛЬ ПИСЬМА:
- Программы описывают внутренние убеждения человека от 3-го лица
- Используй имя {username} в программах
- Конкретно, без "возможно" и "может быть"
- Как на живой консультации, 7-10 минут глубины

Примеры ПРАВИЛЬНЫХ программ (от 3-го лица с именем):
- «{username} не может/не может жить без её любви»
- «Без признания других {username} ничего не стоит»
- «{username} не достойна/достоин большего»
- «Ценность {username} зависит от оценки других»

Примеры НЕПРАВИЛЬНЫХ (не используй):
- «Я не могу» ❌
- «Я не достойна» ❌
- «Моя ценность» ❌
...
"""
        ...
        response = self.client.chat.completions.create(
            ...
            temperature=0.7,
            max_tokens=2500  # ТЕСТ: увеличили в 3 раза для максимальной глубины
        )

        return response.choices[0].message.content, response.usage

    def _step2_ancestral_patterns(self, request_text: str, step1_result: str, username: str) -> tuple:
...
1. КОНТРАКТ (одно предложение - прямая цитата в кавычках от 3-го лица с использованием имени {username})
   ВАЖНО: Контракт должен быть от 3-го лица, НЕ от 1-го лица!
   Примеры ПРАВИЛЬНО:
   - «{username} не достойна/достоин жить для себя»
   - «{username} не может/не может быть счастливой/счастливым без её любви»
   - «{username} должна/должен отдавать всё другим»

   Примеры НЕПРАВИЛЬНО (не используй):
   - «я не могу» ❌
   - «я не достойна» ❌
...
"""
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[{"role": "user", "content": prompt}],
            temperature=0.7,
            max_tokens=3000  # ТЕСТ: увеличили в 3 раза для глубокой расшифровки
        )
...
"""
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[{"role": "user", "content": prompt}],
            temperature=0.7,
            max_tokens=2500  # ТЕСТ: увеличили в 3 раза для детального анализа чакр
        )
...
"""
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[{"role": "user", "content": prompt}],
            temperature=0.8,
            max_tokens=2000  # ТЕСТ: увеличили для более глубоких трансформационных фраз
        )
...
[из РАСШИФРОВКИ части 2: как влияло в прошлом, какие решения принимались, 2-3 предложения. Пример: "{username} постоянно ставил(а) потребности других выше своих..."]

• Как это влияет сейчас
[из РАСШИФРОВКИ части 2: где это проявляется в жизни человека, конкретные примеры поведения, 2-3 предложения. Пример: "{username} продолжает испытывать хроническую усталость... {username} притягивает людей, которые не ценят..."]

• Как это может повлиять в будущем
[из РАСШИФРОВКИ части 2: к чему приведёт, если не изменить программу, 1-2 предложения. Пример: "Если не изменить программу, {username} может продолжать чувствовать себя недооценённой..."]
...
- ОБРАЩЕНИЕ: Пиши о человеке в 3-м лице, используя имя "{username}" с правильными падежами. НЕ используй "ты/тебе/тебя" - только "{username} испытывает", "у {username} есть", "{username}" и т.д.
...
"""
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[{"role": "user", "content": prompt}],
            temperature=0.7,
            max_tokens=6000  # ТЕСТ: увеличили в 2 раза для максимально глубокой компоновки
        )

        return response.choices[0].message.content, response.usage


# Функция для использования в боте (ТЕСТОВАЯ ВЕРСИЯ С УВЕЛИЧЕННЫМИ ТОКЕНАМИ)
async def analyze_with_metamethod_test(request_text: str, username: str) -> tuple:
    """
    ТЕСТОВАЯ версия с увеличенными токенами для максимальной глубины
